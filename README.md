...
기관 · 인물 · 통계 아님). 내용을 수정하려면 `tools/generate_materials.py`를 고친 뒤 아래 명령으로
다시 생성합니다. 각 파일은 공공기관 첨부 용량 제한을 고려해 1MB 이하로 유지되도록 설계되어
있습니다.
...
(`C:\Windows\Fonts\malgun.ttf`, `malgunbd.ttf`)을 사용합니다. 다른 OS에서 실행하려면 스크립트 상단의
`FONT_REG` / `FONT_BOLD` 경로를 해당 환경의 한글 TTF 폰트로 바꿔주세요.

## 프로젝트 구조

- `src/data/curriculum.ts` — 60분 교안 데이터(6개 모듈: 목표 · 시나리오 · 프롬프트 · 첨부자료 · 진행순서 · 진행자 Tip)
- `src/components/PromptBlock.tsx` — 실습용 프롬프트 복사 메뉴바(클립보드 복사) 컴포넌트
...
- `tools/generate_materials.py` — 실습자료 생성 스크립트
